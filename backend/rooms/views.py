...
def rooms_for_building(request):
    ...
    results = []
    for room in rooms:
        occupied = getattr(room, 'has_overlapping_reservation', False)
        results.append({
            "id": room.pk,
            "text": format_room_option(room),
            "occupied": occupied,
        })
...
def room_availability(request):
    """
    NEW: Returns available/booked dates for a specific room.
    Expects query param `room_id` (id).
    Returns JSON:
      {
        "booked": [
          {"from": "2025-01-15", "to": "2025-01-20"},
          ...
        ]
      }
    """
    ...
    # Get all active reservations for this room (not cancelled, not checked out)
